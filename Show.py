...
class Menu:
    ...
    def selection(self, cat):
        ...
        for id_product in idp:
            id_product = str(id_product).replace(',', '').\
                replace('(', '').replace(')', '')
    ...
    def sub(self, cat):
        ...
        select = input("Do you want to have substitue?\n"
                       "o: Oui\n"
                       "n : Non\n")
        if select == "o":
            substitue = self.session.query(Products).\
                filter_by(categorie_id=cat).\
                order_by(Products.name_nut).first()
            print(substitue)
            save = input("Do you want to save the substitue?\n"
                         "o: Oui\n"
                         "n : Non\n")
            id_sub = self.session.query(Products.id).\
                filter_by(categorie_id=cat).\
                order_by(Products.name_nut).first()
            id_sub2 = str(id_sub).replace(',', '').\
                replace('(', '').replace(')', '')
    ...
    def show_substitue(self):
        ...
        products = self.session.query(Products).\
            join(Substitue, Substitue.products_ID == Products.id)
        substitue = self.session.query(Products).\
            join(Substitue, Substitue.substitue_ID == Products.id)
